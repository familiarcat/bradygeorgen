...
export default function LoveTypeDetail({ typeId }: { typeId: string }) {
  const loveType = loveTypes.find(
    (l) => l.id.toLowerCase() === typeId.toLowerCase()
  );

  if (!loveType) {
    return (
      <div className="p-6">
        <Link href="/love-visualization/" className="text-blue-600 hover:underline">
          Back to overview
        </Link>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <Link href="/love-visualization/" className="text-blue-600 hover:underline block mb-4">
        Back to overview
      </Link>
      <div
        className="rounded-lg p-8 text-white mb-8 shadow-lg"
        style={{ backgroundColor: loveType.color }}
      >
      ...
      <div className="prose prose-lg max-w-none">
        ...
        <div className="mt-8 p-6 bg-gray-800 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {loveTypes
              .filter(l => l.id !== loveType.id)
              .slice(0, 2)
              .map(related => (
                <Link
                  key={related.id}
                  href={"/love-visualization/" + related.id.toLowerCase() + "/"}
