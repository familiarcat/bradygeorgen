...
export function generateStaticParams() {
  return loveTypes.map((love) => ({
    type: love.id.toLowerCase(),
  }));
}

export function generateMetadata({ params }: any) {
  const loveType = loveTypes.find(
    (l) => l.id.toLowerCase() === (params ? params.type : "")
  );
  const title = (loveType && loveType.label ? loveType.label : "Unknown") + " - Greek Love Type";
  const description = loveType && loveType.description
    ? loveType.description
    : "Learn about the types of Greek love";
