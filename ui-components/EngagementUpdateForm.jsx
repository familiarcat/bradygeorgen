...
import { updateEngagement } from "./graphql/mutations";
...
export default function EngagementUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = engagementRecord
      ? { ...initialValues, ...engagementRecord }
      : initialValues;
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getEngagement.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEngagement
        : engagementModelProp;
  ...
  React.useEffect(resetStateValues, [engagementRecord]);
  const validations = {
    client: [],
    startDate: [],
    endDate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateEngagement.replaceAll("__typename", ""),
            variables: {
              input: {
